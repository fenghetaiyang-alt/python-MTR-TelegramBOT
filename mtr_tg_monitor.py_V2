...
# -*- coding: utf-8 -*-
# 每秒发包，发10个包，暂停5秒，每15秒循环。如果mtr有隐身节点卡住，超时时间35秒。超时后才开始新的循环
...
REMIND_INTERVAL_SEC = 20 * 60            # 持续故障时，每 20 分钟重新提醒一次（保持完美状态）
...
def monitor_link(isp_name, src_ip):
    ...
    # 🩹 参数加固：-m 24 完美覆盖你 20 跳的 ISP；-G 1 限制末尾节点无谓死等。
    # 这样可以强行将 MTR 的耗时压缩，配合 timeout=35 秒提供绝对冗余空间。
...
def main():
    while True:
        # 🩹 【纯异步高并发修正】：抛弃死板的挨个 join 阻塞，允许线程完全并发、各跑各的
        for isp_name, src_ip in LINKS.items():
            t = threading.Thread(target=monitor_link, args=(isp_name, src_ip))
            t.daemon = True # 设置为守护线程，防止卡死主进程
            t.start()
            
        # 🔄 固定每隔 10 秒（INTERVAL_SEC）无条件发起下一轮新拨测
